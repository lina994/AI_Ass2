public class HeuristicCalculator {
    ...
    public static double calculateF(int agentIndex, int agentPosition, int[] people, int numOfPeopleInCar, double time,
    ...
    private static double calculateHeuristic(int agentIndex, int agentPosition, int[] people, int numOfPeopleInCar,
            double time, double personWeight) {
        double result = 0;
    ...
    private static double calculateFullCooperativeHeuristic(StateNode node, double personWeight) {
        ...
        for (int i = 0; i < node.peopleArr.length; i++) {
            if (node.peopleArr[i] > 0) {
                double timeToEvacuateByAgent = calculateTime(node.agentPosition, i, node.peopleArr,
                        node.peopleInAgentCar);
                double timeToEvacuateByOpponent = calculateTime(node.opponentPosition, i, node.peopleArr,
                        node.peopleInOpponentCar);
                if (timeToEvacuateByAgent + node.time > Main.deadline
                        && timeToEvacuateByOpponent + node.time > Main.deadline)
                    result += node.peopleArr[i] * personWeight;
            }
        }
